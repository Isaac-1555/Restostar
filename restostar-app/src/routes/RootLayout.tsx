...
export function RootLayout() {
  ...
  return (
    <div className={isHomePage ? "min-h-full bg-stone-50" : "min-h-full bg-stone-50"}>
      <header
        className={
          isHomePage
            ? "sticky top-0 z-50 border-b border-emerald-950/5 bg-stone-50/85 backdrop-blur-xl"
            : "sticky top-0 z-50 border-b border-emerald-950/10 bg-white/95 shadow-sm backdrop-blur-sm"
        }
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <Link
            ...
            className={`group flex items-center gap-3 transition-colors ${
              isHomePage ? "text-emerald-950" : "text-emerald-950"
            }`}
          >
            <img
              src={logo}
              ...
              className={`h-9 w-9 rounded-full object-cover ring-1 ${
                isHomePage ? "ring-emerald-950/10" : "ring-emerald-200"
              }`}
            />
              ...
              <Link
                ...
                className={`inline-flex items-center rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-all ${
                  isHomePage
                    ...
                    : "bg-emerald-950 hover:bg-emerald-900 hover:shadow-md"
                }`}
              >
                Get Started
              </Link>
